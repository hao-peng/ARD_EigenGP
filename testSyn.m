...
trained_model = EigenGPARD_train(model, x, y, M, 10);


param = EigenGPARD_model2param(trained_model, D, M);

EigenGPARD_negLogLik(param, x, y, M);
% 
% index = 2;
% range = param(index)-1:0.01:param(index)+1;
% for i = 1:size(range,2);
%     param(index) = range(i);
%     [f(i), g] = EigenGPARD_negLogLik(param, x, y, M);
%     df(i) = g(index);
% end
...
[mu s2] = EigenGPARD_pred(trained_model, x, y, xtest);
